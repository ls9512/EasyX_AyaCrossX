...
int is3D=0;						//是否3D效果绘制(EasyX的坑爹bar3d函数尝试)
int mouseX;						//鼠标位置坐标X
int mouseY;						//鼠标位置坐标Y
int time_max;					//总游戏时间
int time_now;					//当前剩余时间
...
{
	if((x>=r.x && x<=r.w) && (y>=r.y && y<=r.h)) return true;
	else return false;
}
//添加制定个数的未选中方块
void AddBlock(int n)
...
void StartDraw()
{
	...
	outtextxy(143, 93, "AyaCrossX");
	setcolor(RGB(255,153,238));
	outtextxy(140, 90, "AyaCrossX");
	//版本
	settextstyle(22, 10, _T("Verdana"));
	setcolor(RGB(255,51,68));
	outtextxy(340, 153,"v0.91 By:ls9512");
	//开始游戏按钮
	Rect r;
	r.x=240;
	r.y=220;
	...
	else setfillcolor(RGB(255,255,204));
	fillrectangle(r.x,r.y,r.w,r.h);
	settextstyle(25, 10, _T("Verdana"));
	setcolor(RGB(255,51,68));
	outtextxy(r.x+5, r.y+5,"PLAY GAME");
	...
	settextstyle(25, 10, _T("Verdana"));
	setcolor(RGB(255,51,68));
	outtextxy(r.x+5, r.y+5,"HELP INFO");
	//退出游戏按钮
	r.x=240;
	r.y=320;
	r.w=r.x+125;
	...
	settextstyle(25, 10, _T("Verdana"));
	setcolor(RGB(255,51,68));
	outtextxy(r.x+5, r.y+5,"EXIT GAME");
	//说明
	settextstyle(16, 5, _T("Verdana"));
	setcolor(RGB(255,51,68));
	outtextxy(175, 400,"Programing By VC++ & EasyX     Date:2013.11.01");
}
//游戏中绘图
void GameingDraw()
{
	...
	setcolor(RGB(255,17,255));
	setfillcolor(WHITE);
	fillrectangle(x,y,x+HPBARW,y+10);
	setfillcolor(C_HPBAR);
	fillrectangle(x,y,x+(int)(HPBARW*(1.0*time_now/time_max)),y+10);
	//底板
	setcolor(RGB(255,25,22));
	setfillcolor(C_BLOCKPANEL);
	x=WIN_BLANK - G_BS;
	y=WIN_BLANK+WIN_BLANKTOP - G_BS;
	if(is3D) bar3d(x,y,x+(G_BW+G_BS)*G_BN+G_BS,y+(G_BW+G_BS)*G_BM+G_BS,3,true);
	else fillrectangle(x,y,x+(G_BW+G_BS)*G_BN+G_BS,y+(G_BW+G_BS)*G_BM+G_BS);
	//画方块
	for(int i=0;i<G_BM;i++)
	{
		for(int j=0;j<G_BN;j++)
		{
			if(!block[i][j].isClick)
			{
				...
				if(is3D) bar3d(x,y,x+G_BW + 2*(fw + fd),y+G_BW + 2*(fw +fd),3,true);
				else fillrectangle(x,y,x+G_BW+2*(fw+fd),y+G_BW+2*(fw+fd));
				//浮动缩减
				if(block[i][j].FLOAT>0) block[i][j].FLOAT-=0.4f;
				//消失延迟
				if(block[i][j].isDIS)
				{
					if(block[i][j].DIS < G_BDW) block[i][j].DIS+=0.4f;
					else{
						//消除
						block[i][j].isClick = true;
						//加分
	...
	if(p.x!=-1 && p.y!=-1)
	{
		...
		if(!block[p.y][p.x].isClick)
		{
			...
			//选中位置的稍微加亮
			//c2=ReRGB(GetRValue(c2)+(BYTE)G_BUL,GetGValue(c2)+(BYTE)G_BUL,GetBValue(c2)+(BYTE)G_BUL);
		}
		else
		{
			c2=C_BLOCKPANEL;
		...
		setcolor(c);
		setfillcolor(c2);
		if(is3D) bar3d(x,y,x+G_BW + 2*(G_BSELECT + fw),y+G_BW + 2*(G_BSELECT + fw),1,true);
		else fillrectangle(x,y,x+G_BW + 2*(G_BSELECT+fw),y+G_BW + 2*(G_BSELECT + fw));
	}
	//画出分数
	settextstyle(25, 10, _T("Verdana"));
	setcolor(RGB(155,51,68));
	char b[10];
	sprintf_s(b,"%d",score);
	outtextxy(390, 15, "分数:");
	setcolor(RGB(255,51,68));
...
void HelpDraw()
{
	...
	setcolor(RGB(245,126,92));
	outtextxy(70, 110,"十字消除是一款经典消除类小游戏。");
	outtextxy(70, 145,"【玩法】");
	outtextxy(80, 170,"1.点击空白处,所在横纵十字线上同色方块会被消除。");
	outtextxy(80, 195,"2.消除方块会累加得分,并恢复一定量的时间.");
	outtextxy(80, 220,"3.方块数量少于某数值会出现新方块并回复时间.");
	outtextxy(80, 245,"4.时间耗尽时则游戏结束.");
	//返回
	Rect r;
	r.x=240;
	r.y=360;
	...
	setcolor(RGB(255,51,68));
	outtextxy(r.x+5, r.y+5,"BACK MENU");
}
//输出帧数
void DrawFPS()
{
...
int main()
{
	...
	//设置窗口标题
	SetWindowText(GetHWnd(),"AyaCrossX v0.9 By:ls9512");
	//清除设备
	cleardevice();
	//设置黑色背景
